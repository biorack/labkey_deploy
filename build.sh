...
# Check if "--image backup_restore" was passed as a parameter
if [[ "$*" != *"--image backup_restore"* ]]; then
    echo "Error: Use the '--image backup_restore' parameter to run script"
    echo "This script is only used to build the backup_restore image and not the labkey image"
    exit 1
fi
...
ARCH="macM2" # Building on new Mac architecture requires 'macM2', else leave blank
...
if [[ "$ARCH" == "macM2" ]]; then
  ${DOCKER} buildx build --platform=linux/arm64 --load --tag "${SHORT_TAG}" "${DOCKERFILE_DIR}"
else
